...
if($requestMethod!="POST" && $requestMethod!="PUT"){
	...
	//var_dump($input);
	if($input['following_id']=='' || $input['follow_by_id']==''){
		$response_data = array("response"=>"All fields are mandatory");
	} else {
		include '../../db.php';
		$create_time = date('Y-m-d H:i:s');
		
		$sql = "SELECT * FROM `tbl_user_info` WHERE id = '".$input['following_id']."'";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			$row = $result->fetch_assoc();
			$follower_list = $row["followers_id_list"];
			...
			if ($result_fby->num_rows > 0) {
				$sql_update = "UPDATE `tbl_user_info` SET followers_id_list = '".$follower_list."' WHERE id = '".$input['following_id']."'";
			...
			$response_data = array("response"=>"The user you wants to follow, is invalid");
		}
		$conn->close();
	}
}
